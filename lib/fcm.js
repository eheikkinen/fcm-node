...
function FCM(accountKey, proxy_url=null, name=null) {
    ...
    if(!accountKey) {
        ...
        var appOptions = {}
        if (proxy_url) {
            // same HttpAgent required to be set for the credential and AppOptions for Messaging API
            var httpAgent = new HttpsProxyAgent(proxy_url)
            appOptions.credential = firebaseadmin.credential.cert(accountKey, httpAgent)
            appOptions.httpAgent = httpAgent
        } else {
            appOptions.credential = firebaseadmin.credential.cert(accountKey)
        }

        if(name) {
            firebaseadmin.initializeApp(appOptions, name);
        } else {
